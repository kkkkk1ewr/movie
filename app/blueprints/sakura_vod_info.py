...
mov_type_dict = {
    1: [1, 5, 6, 7, 8, 9, 10, 11, 20, 41, 61],
    2: [2, 12, 13, 14, 15, 16, 17, 18, 54, 82],
    3: [3, 62, 63, 64, 65],
    4: [4, 66, 67, 68, 69, 70],
    # 5: [5, 17, 18],
    0: [1, 2, 3, 4]
...
def get_vod_list():
    ...
    if type_name:
        movs = MovDetail.query.filter(MovDetail.type_name.like(f"%{type_name}%"))
    else:
        movs = MovDetail.query.filter(MovDetail.type_id_1.in_(mov_type_list))
    ...
    if vod_area:
        if vod_area != '其他':
            ...
            movs = movs.filter(
                MovDetail.vod_area.notin_(["大陆", "美国", "日本", "韩国", "英国", "法国", "香港", "泰国"]))
